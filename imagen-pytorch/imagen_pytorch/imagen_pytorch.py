...
class CosineSimilarityLoss(nn.Module):
    def forward(self, target_word_emb, word_emb_to_compare):
        cosine_sim = F.cosine_similarity(target_word_emb, word_emb_to_compare)
        loss = 1 - cosine_sim  # Subtract from 1 to convert similarity to a loss
        return loss.mean()  
...
class Imagen(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.text_loss_fn = CosineSimilarityLoss()	
    ...
    def p_losses(
        ...
        **kwargs
        ...
        # get prediction
        print('getting normal prediction')
        pred = unet.forward(
            x_noisy,
            noise_cond,
            **unet_kwargs
        ...
        print('getting text acc')
        pred_imgs = self.sample(text_embeds=text_embeds, batch_size=4, use_tqdm = False, return_pil_images = True, cond_scale=5.)

        total_dist = 0
        pred_texts = []
        for i, img in enumerate(pred_imgs):
            #target_text_emb = text_embeds[i]
            #matches = re.findall(r"'([^']*?[^\\])'", target_text)
            #print(matches[0])
            #target_text = matches[0] if len(matches) > 0 else " "
            try:
                #print(pytesseract.image_to_string('test.jpg', timeout=2)) # Timeout after 2 seconds
                pred_text = pytesseract.image_to_string(img, timeout=0.5).strip() # Timeout after half a second
            except RuntimeError as timeout_error:
                # Tesseract processing is terminated
                pred_text = "[unknown]"

            # pytesseract.image_to_string(img)
            #pred_text_emb = self.encode_text(f"the text '{pred_text}'")
            pred_texts.append(f"the text '{pred_text}'")
            #print(f"pred: '{pred_text_emb.shape}', target: '{target_text_emb.shape}'")
            #total_dist += editdistance.eval(pred_text, target_text) / len(target_text)
            #img.save(f'./pred-{i}.png')

        pred_text_embeds = self.encode_text(pred_texts)
        
        print(f"target text embed shape: {text_embeds.shape}, pred text embeds shape {pred_text_embeds.shape}")
        text_loss = self.text_loss_fn(pred_text_embeds, text_embeds)
        print(f'finished text acc {total_dist}')
        ...
        print(f'regular losses = {losses.mean().shape}')

        return losses.mean() + (1.5 * text_loss)
    ...
    def forward(
        ...
        **kwargs
        ...
        return self.p_losses(unet, images, times, texts = texts, text_embeds = text_embeds, text_mask = text_masks, cond_images = cond_images, noise_scheduler = noise_scheduler, lowres_cond_img = lowres_cond_img, lowres_aug_times = lowres_aug_times, pred_objective = pred_objective, min_snr_gamma = min_snr_gamma, random_crop_size = random_crop_size, **kwargs)
